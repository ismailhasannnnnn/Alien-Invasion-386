...
class Ship(Sprite):
  exploding_images = [pg.transform.rotozoom(pg.image.load(f'images/frame_{n}_delay-0.07s.png'), 0, 0.23) for n in range(9)]
  ...
  def __init__(self, game):
    ...
    self.firing = False
    self.frames = 0
    self.exploding_timer = Timer(image_list=Ship.exploding_images, delay=70, is_loop=False)
  ...
  def update(self):
    ...
    if self.frames % 10 == 0 and self.firing and not self.is_dying():
      self.lasers.fire()
    self.frames += 1
